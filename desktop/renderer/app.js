...
function initUpdater() {
    ...
    window.electronAPI.onUpdateStatus((data) => {
        ...
        if (data.status === 'available') {
            ...
            let errorMessage = t.updates.errorGeneric.replace('{error}', 'Unknown error');
            if (data.error) {
                if (data.error.includes('network') || data.error.includes('ENOTFOUND') || data.error.includes('ECONNREFUSED')) {
                    errorMessage = t.updates.errorNetwork;
                } else if (data.error.includes('404') || data.error.includes('Not Found')) {
                    errorMessage = t.updates.errorNotFound;
                } else if (data.error.includes('rate limit') || data.error.includes('403')) {
                    errorMessage = t.updates.errorRateLimit;
                } else {
                    errorMessage = t.updates.errorGeneric.replace('{error}', data.error);
                }
            }
